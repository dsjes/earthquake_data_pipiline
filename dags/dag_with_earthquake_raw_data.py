...
def upload_to_S3(local_source_data_path, S3_file_path):
    with s3_session() as s3:
        result = s3.Bucket("for-side-project-demo").upload_file(
            local_source_data_path, S3_file_path
        )
...
def get_earthquake_data(**context) -> dict:
    EARTHQUAKE_API_KEY = os.getenv("EARTHQUAKE_API_KEY")

    start_time = context["ti"].xcom_pull(
        task_ids="transform_datetime_format", key="start_time"
    )
    end_time = context["ti"].xcom_pull(
        task_ids="transform_datetime_format", key="end_time"
    )

    url = f"https://opendata.cwa.gov.tw/api/v1/rest/datastore/E-A0015-001?Authorization={EARTHQUAKE_API_KEY}&timeFrom={start_time}&timeTo={end_time}"
    ...
    if response.status_code == 200:
        ...
        for earthquake in earthquake_results:
            ...
            local_source_data_path = (
                f"/opt/airflow/data/earthquake_{earthquake_number}_source_data.json"
            )

            S3_file_path = f"earthquake_source_data/earthquake_{earthquake_number}_source_data.json"
            write_source_json_data(
                local_source_data_path=local_source_data_path, json_object=earthquake
            )

            if os.path.isfile(local_source_data_path) and not file_exists_on_S3(
                key=S3_file_path
            ):
...
with DAG(
    ...
    task_get_earthquake_data = PythonOperator(
        task_id="task_get_earthquake_data",
